...
function FetcherMetroLinies({ children }) {
  ...
  useEffect(() => {
    ...
    fetch('https://api.tmb.cat/v1/transit/linies/metro?app_id=b2840f13&app_key=8d96b390bd3dab7a484de10fbce6e435&limit=15') // Fem una crida a l'API de linies de metro
      .then(response => {
        if (!response.ok) {
          throw new Error('Error al recollir dades: Linies de Metro'); // Llençar error si la resposta no és ok
        }
...
export default FetcherMetroLinies;